...
import {
  ...
  PATIENT_MR_FAIL,
  PATIENT_MR_RESET,
  PATIENT_REGISTER_TR_REQUEST,
PATIENT_REGISTER_TR_SUCCESS,
PATIENT_REGISTER_TR_FAIL   
...
export const register = (name, email, MRno, age, dept) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(
      process.env.REACT_APP_DB_PATIENT + '/api/patient',
...
export const getPatientDetails = (id) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(process.env.REACT_APP_DB_PATIENT + `/api/patient/${id}`, config)
...
/////////////////
export const getPatientMR = () => async (dispatch, getState) => {
  try {
    dispatch({
      type: PATIENT_MR_REQUEST,
    })
    ...
    const { data } = await axios.get(process.env.REACT_APP_DB_PATIENT + `/api/patient/getAllMR`)

    dispatch({
      type: PATIENT_MR_SUCCESS,
      payload: data,
    })
  } catch (error) {
    const message =
...
export const updatePatientProfile = (user) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.put(process.env.REACT_APP_DB_PATIENT + `/api/users/profile`, user, config)
...
export const listPatients = () => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(process.env.REACT_APP_DB_PATIENT + `/api/patient`, config)
...
export const deletePatient = (id) => async (dispatch, getState) => {
  try {
    ...
    await axios.delete(process.env.REACT_APP_DB_PATIENT + `/api/patient/${id}`, config)
...
export const updatePatient = (user) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.put(process.env.REACT_APP_DB_PATIENT + `/api/patient/${user._id}`, user, config)
